...
# model_vit_path = '/root/bifirn-tran/vit_base_patch16_224_in21k.pth'
...
class FRN(nn.Module):
    ...
    def __init__(self,way=None,shots=None,resnet=False,is_pretraining=False,num_cat=None):
        ...
        if resnet:
            ...
            num_channel_conv = 64
            self.feature_extractor_conv = Conv_4.BackBone(num_channel_conv)
            self.feature_extractor_conv = nn.DataParallel(self.feature_extractor_conv, device_ids=[0, 1, 2])

        else:
            num_channel_conv = 64
            self.feature_extractor = Conv_4.BackBone(num_channel_conv)
        ...
        self.d_conv = num_channel_conv
        self.d_res = num_channel_res
        ...
        self.resolution = 25
        self.cov = nn.Conv2d(self.d_res,self.d_conv, kernel_size=1, stride=1)
    ...
    def get_feature_map(self,inp):
        # inp = self.transform_84(inp)
        batch_size = inp.size(0)
        feature_map = self.feature_extractor.module(inp)
        feature_map_conv = self.feature_extractor_conv.module(inp)
        
        if self.resnet:
            feature_map = feature_map/np.sqrt(640)

        feature_map_avg = self.maxpool(feature_map)
        feature_map_avg = self.cov(feature_map_avg)
        feature_map_conv_avg = self.maxpool(feature_map_conv)

        z_cls_feature = torch.cat((feature_map_avg, feature_map_conv_avg), dim=1)
        z_cls_feature = z_cls_feature.view(z_cls_feature.size(0), z_cls_feature.size(1))
        w_count = F.softmax(z_cls_feature, dim=1)
        sum1 = w_count[:, :64].sum(dim=1)
        sum2 = w_count[:, 64:].sum(dim=1)
        feature_map = self.w1 * feature_map * sum1.view(w_count.size(0), 1, 1, 1)
        feature_map_conv = self.w2 * feature_map_conv * sum2.view(w_count.size(0), 1, 1, 1)

        feature_map = feature_map.view(batch_size,self.d_res,-1).permute(0,2,1).contiguous() # N,HW,C
        feature_map_conv = feature_map_conv.view(batch_size,self.d_conv,-1).permute(0,2,1).contiguous() # N,HW,C
        
        return feature_map, feature_map_conv
    ...
    def get_neg_l2_dist(self,inp,way,shot,query_shot,return_support=False):
        ...
        feature_map, feature_map_conv = self.get_feature_map(inp)

        support = feature_map[:way*shot].view(way, shot*resolution , d_res)
        query = feature_map[way*shot:].view(way*query_shot*resolution, d_res)

        support_conv = feature_map_conv[:way*shot].view(way, shot*resolution , d_conv)
        query_conv = feature_map_conv[way*shot:].view(way*query_shot*resolution, d_conv)

        recon_dist_res = self.get_recon_dist(query=query,support=support,alpha=alpha,beta=beta) # way*query_shot*resolution, way
        neg_l1_dist = recon_dist_res.neg().view(way*query_shot,resolution,way).mean(1) # way*query_shot, way

        recon_dist_conv = self.get_recon_dist(query=query_conv, support=support_conv, alpha=alpha,
                                             beta=beta)  # way*query_shot*resolution, way
        neg_l2_dist = recon_dist_conv.neg().view(way * query_shot, resolution, way).mean(1)  # way*query_shot,
        neg_l_dist = (self.w3 * neg_l1_dist) * (self.w4 * neg_l2_dist)
